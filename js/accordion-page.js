/* ═══════════════════════════════════════════════
   CLOVER OOC - ACCORDION PAGE JS
   Shared engine for the accordion + card + search pages
   (Scenes, Restyle). Each page supplies its data, element
   ids and localized strings; all behaviour lives here.
   aceenvw
   ═══════════════════════════════════════════════ */

window.cloverAccordionPage = function cloverAccordionPage(config) {
  'use strict';

  const data = config.data;
  if (!data || !Array.isArray(data.sections)) {
    console.error(config.label + ' data not loaded');
    return;
  }
  ...
  function buildCard(prompt, sectionId) {
    ...
    bodyEl.setAttribute('aria-label',
      getLang() === 'ru' ? config.copyLabel.ru : config.copyLabel.en);
